...
\begin{center}
  ...
  A ficticious particle, on a circular orbit coplanar with the circumtriple disk a \todo{XX} au., Representative circular disk orbit shown as grey dashed line.
  \label{fig:astro}}
\end{center}
\end{figure*}
...
This analysis produces consistent but less precise constraints on the stellar masses as the joint RV + astrometric fits (see Table~\ref{table:component_masses}). Like the RV + astrometric analysis, the disk + RV analysis also indicates that the inner stellar orbit must be significantly misaligned with the disk, since the difference between $i_\mathrm{in}$ and $i_\mathrm{disk}$ provides a lower limit on the mutual inclination because the true mutual inclination must consider the position angles of the orbits as well. To highlight these findings, we overplot our newly derived constraints on the disk inclination and the stellar orbits in Figure~\ref{fig:incl_posterior}.
Interestingly, the constraints on $i_\mathrm{in}$ differ between the RV + astrometry and the RV + disk results at a significant level. We speculate that this difference could be due to unknown systematics in the astrometry or RV datasets, or potentially if our assumption about the distance to \gw\ is incorrect, and that it might be at a further distance. This is because the astrometry + RV analysis does not require a distance to the source, while the disk-based analysis does require a distance in order to break the $M_\mathrm{tot} / d$ degeneracy.
