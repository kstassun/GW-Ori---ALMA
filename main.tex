...
\begin{center}
  ...
  \figcaption{\emph{top}: The photometric observations of GW Ori from 1987 until mid 2017. All photometric observations displayed here are in the $V$-band (ASAS, ASAS-SN, and Grankin et al.) or a broader filter (KELT) which has been shifted to align with $V$-band where the time series overlap. \emph{bottom}: primary and secondary radial velocities overlaid with several realizations of the most probable orbits, to show uncertainty in the orbit. Reticon velocities are shown with square points, TRES velocities are shown with circles, and the dotted line represents the center-of-mass velocity. Residuals for this orbit are shown in the panels below.
  \label{fig:orbitboth}}
\end{center}
\end{figure*}
...
Although there are only three epochs of published astrometry in \citet{berger11}, these points may still considerably constrain the parameter space of possible orbits. Therefore, we explore a joint RV-astrometric analysis built upon a model of the ``three-dimensional orbit'' following \citet{murray10}, which adds new model parameters like the semi-major axis, inclination, and position angle on the sky for both inner and out orbits. For a likelihood function, we combine the $\chi^2$ radial velocity likelihood and a $\chi^2$ likelihood function for the separation and azimuth measurements. As with the disk analysis, we also use a geometric prior on the orbital inclinations. For their last epoch of measurements in 2005, \citet{berger11} report an alternate position for the C component, and so we do a separate fit considering that this is the true location of C. The newly constrained parameters are in the second and third columns of Table~\ref{tab:elements}.
With the addition of astrometric constraints, we are able to constrain the masses of individual stellar components and the inclinations of the orbital planes, which are also listed in Table~\ref{tab:elements}. Depending on whether the reported or alternate postition for C is used, we find the total mass to be $M_\mathrm{tot} = 5.7 \pm 0.7$ or $M_\mathrm{tot} = 6.1 \pm 0.9$, respectively, both of which are nicely consistent with that independently measured by the disk-based analysis.
...
\enddata
\tablenotetext{a}{We fit for a potential velocity offset between the primary and secondary radial velocities. In principle, this term should be consistent with $0$; the non-zero value likely indicates that there is some moderate template mismatch between the secondary stellar spectrum and the synthetic spectrum used as a cross correlation template.}
