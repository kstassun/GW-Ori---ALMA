...
\author{KELT Co-authors}
%Michael B. Lund    Department of Physics and Astronomy, Vanderbilt University, 6301 Stevenson Center, Nashville, TN 37235, USA
%Rudolf B. Kuhn     South African Astronomical Observatory, P.O. Box 9, Observatory 7935, South Africa
%Daniel J. Stevens  Department of Astronomy, The Ohio State University, Columbus, OH 43210, USA
%Robert J. Siverd   Las Cumbres Observatory Global Telescope Network, 6740 Cortona Drive, Suite 102, Santa Barbara, CA 93117, USA
%David James        Astronomy Department, University of Washington, Box 351580, Seattle, WA 98195, USA
%B. Scott Gaudi     Department of Astronomy, The Ohio State University, Columbus, OH 43210, USA
...
We compile a lightcurve using several sources of data.

Multi-filter (UBVR) photometry acquired by K.N.~Grankin at Maidanak Observatory in Uzbekistan, from 1987 until 2003.

V-band photometry acquired by ASAS spanning from XX to XX.

KELT photometry, spanning XX to XX.

\todo{KELT photometry. Description of KELT, reduction procedures, and a figure showing the full span of the lightcurve.}

\todo{Photometry as machine readable table}.


\subsubsection{KELT}
The kilodregree extremely little telescope (KELT) project uses two telescopes to survey over 70\% of the entire sky searching for transiting planets around bright stars (8$<$V$<$11). The telescopes, located in Sonoita, AZ (KELT-North) and Sutherland, South Africa (KELT-South), have a 42mm Mamiya 645-series wide-angle lens resulting in a $26^{\circ}$ $\times$ $26^{\circ}$ field-of-view (FOV), and a 23$\arcsec$ pixel scale. Additionally, both telescope use a broad $R$-band filter. GW Ori is located in KELT-South field 05 ($\alpha$ =  06hr 07m 48.0s, $\delta$ = $+3^{\circ}$ 00$\arcmin$ 00$\arcsec$) and observed from UT 2010 February 28 until UT 2015 April 09, obtaining 2889 observations after reduction. See \citet{Siverd12} and \citet{Kuhn16} for a detailed description of the KELT observing strategy and reduction process. KELT-South achieves a median per-point error of 0.005 mag on GW Ori.

\subsubsection{ASAS}

% AAVSO results are not being shown, because they are too noisy. 
...
\enddata
\tablecomments{The 1-D marginal posteriors are all well described by a Gaussian, and so we report symmetric error bars here.}
\end{deluxetable}
...
\begin{center}
  ...
  \figcaption{Posterior distributions for the model parameters fit to the \thirteen\ and \eighteen\ data independently, showing 1 and 2 $\sigma$ contours. Dashed lines indicate constant values of $M_\mathrm{tot} \sin^2 i_\mathrm{disk}$, to highlight the origin of the correlated posteriors. \label{fig:posterior}}
  \end{center}
\end{figure}

The inferred parameter values corresponding to the measurements of each spectral line are summarized together in Table~\ref{table:components}.  A comparison of the data and the best-fit models (and associated residuals) is presented in the form of channel maps in Figures~\ref{fig:13co} and \ref{fig:c18o} for $^{13}$CO and C$^{18}$O, respectively.  While overall the models successfully reproduce the observed emission, there are some interesting residuals \todo{highlight velocity ranges}.  We will return to a discussion of their potential origins in Section~\ref{sec:discussion}.  

The inferred physical structures for each line are in mild disagreement, as might be expected for an over-simplified model based on data products that are sensitive to different regions of the disk. To illustrate the slightly different disk structures inferred for each transition, we plot the 2D temperature and density disk structures in Figure~\ref{fig:temp_dens}. We attribute these differences to the different opacities of the \thirteen\ and \eighteen\ transitions, which results in probing different depths of the disk structure. \todo{Write about how we tried many different vertical structure models (Rosenfeld), and more flexible models, but these did not yeild more satisfactory results as concerns evaluation with the AIC, although the inferred stellar masses and inclinations were similar.}. With more computational power, it would be worthwhile to explore a simultaneous fit to both of these structures with a more flexible model. 

In Figure~\ref{fig:posterior}, we plot the marginalized posteriors for both transitions in the \{$M_\ast$, $i_\mathrm{disk}$\}-plane. Interestingly, the different transitions result in different inclinations at a statistically significant level ($\Delta i_\mathrm{disk} \approx 2.5^\circ$), which we attribute to the previously mentioned model deficiencies. Nevertheless, analysis of both transitions yields effectively identical constraints on the total stellar mass, which is the most relevant parameter to our stated goals. The robustness of the dynamical mass measurement is because the kinematic structure of the line emission has very little dependence on the density and temperature parameters \citep[see][]{rosenfeld12a,czekala15a}.


\begin{figure*}[ht!]
\begin{center}
  \includegraphics{temperature_and_density.pdf}
  \figcaption{The maximum likelihood 2D temperature (top) and density (bottom) disk structures inferred using the \thirteen\ (left) and \eighteen\ (right) transitions. The temperature contours are in units of K. The density plots show the total gas density ($\rho_\mathrm{gas}$) and are in units $\log_{10} \mathrm{cm}^{-3}$. The color scales are normalized to be the same for both transitions.
  \label{fig:temp_dens}}
  \end{center}
\end{figure*}


Although the inclinations are different, we combine the inferred total masses from \thirteen\ and \eighteen\ in a weighted mean to get $M_\mathrm{tot} = 5.29 \pm 0.06\,M_\odot$ and $i_\mathrm{disk} = 137.6 \pm 0.3^\circ$. We adopt a systematic error on each measurement of \todo{XX}.

% There are several topics we need to discuss related to the optical spectra.

% Discussion of how the orbit of Berger 11, and the infrared detection by L. Prato suggested that we look for the optical spectroscopic light.

% We utilized the PSOAP tools, which converged to a spectroscopic binary orbit. Then, we extended it to use a triple orbit.

% Simultaneously, we also used TODCOR to search for the signatures in the cross-correlation function.
...
\tablewidth{0pc}
\tablecaption{Spectroscopic orbital elements of \gw. \todo{To be updated with SB2 orbit.} \label{tab:elements}}
\tablehead{
...
$M_{\rm C} \sin i / ((M_A + M_B + M_C) / M_\odot)^{2/3} \; $[$M_{\sun}$]\dotfill            &     0.1539~$\pm$~0.0088 \\
...
%
\figcaption[]{Radial-velocity measurements of \gw\ and best-fit model for the inner orbit, after subtracting the motion in the outer orbit. The dotted line in the top panel represents the center-of-mass velocity of the triple system. The bottom panel displays the residuals.\label{fig:orbitin}}
%
\end{figure}
...
%
\figcaption[]{Radial-velocity measurements of \gw\ and best-fit model for the outer orbit, after subtracting the motion in the inner orbit. The dotted line in the top panel represents the center-of-mass velocity of the triple system. The bottom panel displays the residuals.\label{fig:orbitout}}
%
\end{figure}
...
\plotone{figout1.eps}
%
\figcaption[]{Radial-velocity measurements of \gw\ as a function of time, and best-fit model for the triple system. The dotted line represents the center-of-mass velocity.\label{fig:orbitboth}}
%
\end{figure*}
...
As mentioned in the previous section, we also performed a joint RV-astrometric fit with the \citet{berger11} measurements. When we combine the constraints on the orientation of the orbital planes with that of the protoplanetary disk, we are lead to the remarkable conclusion that the planes of the stellar orbits are misaligned by $45 \pm 5^\circ$ with respect to the protoplanetary disk. One can also infer that the disk and outer orbital planes are misaligned without resorting to a full joint RV-astrometric fit, which may be susceptible to systematic errors between the data sets. Using only the CfA RV dataset and a wide range of permissible orbital elements, one would infer that at the first epoch (2003) of the \citet{berger11} astrometric measurements the tertiary would be moving with a large positive radial velocity (relative to the systemic velocity) and by the last epoch (2005) it would be moving at approximately the systemic velocity, suggesting that the line of nodes for the outer orbit is close to the E-W axis. However, our disk results clearly demonstrate that the line of nodes for the disk is the N-S axis, suggesting that these orbits must be misaligned. There is a $90^\circ$ difference between the $\Omega$ convention (defined by the ascending node) and our disk position angle ($\varphi$) convention, which uses the projection of the disk angular momentum vector onto the sky.

\begin{deluxetable}{lr@{\,$\pm$\,}ll}
\tablecaption{Combined disk and RV constraints on component masses \todo{to be updated} \label{table:component_masses}}
...
\begin{center}
  ...
  \figcaption{\emph{left}: GW~Ori~A placed on the early Hertzsprung Russell diagram with the MIST evolutionary models. The mass tracks are 1 - 5 $M_\odot$ in intervals of 1 $M_\odot$, and the dotted lines denote isochrones for $0.1$, $1$, and $10$~Myr. \emph{right}: The joint $M_A$ vs. $\tau$ posteriors with 1 and 2$\sigma$ contours, evaluated using a suite of pre-main sequence evolutionary model predictions. The top and right edges show the marginal mass and age posteriors, respectively. The mass inferred from the photospheric properties is in slight tension with our measured dynamical mass at the $1-2\sigma$ level.
  \label{fig:PMS}}
  \end{center}
\end{figure}
...
\todo{CO sub-mm excess seen in \thirteen\ and \eighteen.}

\todo{Fundamental CO emission}
...
% Discussion of GW~Ori disk mass in context of \citet{andrews13}.
