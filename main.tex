...
% narrowing (short) paragraph}}: general relevance of multiple stars; comment on lingering uncertainties in their formation (especially higher-order multiples).
\todo{Ian editing}
Pre-main sequence binary stars are extremely useful for understanding the earliest stages of a star and planetary system lifetime. Understanding the formation and evolution of multiple star systems is important to the planet formation process generally. While recent decades have seen steady progress in understanding binary formation in general, uncertainties still remain about the formation of higher order multiple systems. \todo{citations}.
...
The \gw\ system was among the first T Tauri star systems to be revealed as a spectroscopic binary \citep{mathieu91}.  With a composite spectral type of late G, it is an SB1 with a nominal period of 240 days. Long term radial velocity monitoring revealed the presence of a third body in the system with a period of 4000 days, and was subsequently confirmed by infrared interferometry \citep{berger11}. Dynamical arguments suggest that the A component is quite massive ($M_A > 3.0\, M_\odot$), and so therefore the system must be quite young if the composite spectral type is still late G. \ion{Mg}{2} observations from IUE and HST constrain magnetospheric properties \citep{lopez-martinez15}.  Recent VLBA parallax measurements have determined the distance to the Orion Nebula Complex to be $388\pm5\,$pc \citep{kounkel16}.
...
Long term photometric and high resolution spectroscopic monitoring has revealed the system to be fascinatingly variable. \citep{shevchenko92,shevchenko98} found quasi-period eclipse-like behavior, possibly from material in the surrounding protoplanetary disk. \todo{correspond with Joey about KELT dimmings}. \citep{mathieu95} detected a strong sub-mm disk, suggesting a massive ($M_\mathrm{disk}/M_\ast > 0.1$) and extended disk. \citep{fang14} find a variable mid-infrared spectral energy distribution (SED), which they believe is evidence of an active inner disk region, that is depleted of and replenished with XX-sized dust on a decade-long timescale. \citep{najita03} and \citep{bast11} present complicated situations of the inner disk, CO fundamental peaks.
...
Evolutionarily, \gw\ presents a system caught at a very interesting stage. If the A component is indeed massive, then the system must be caught at an exceedingly young age ($< 1$\,Myr). On the main sequence, the A component will be a B spectral type star. B stars only have main sequence lifetimes of XX years, and so we are probing a particularly narrow epoch of star formation. If so, \gw\ might represent an earlier stage of analogues recently discovered by \citep{moe15}, which are binaries of main-sequence B stars and \emph{pre-main sequence} T~Tauri stars in the Large Magellanic Cloud. Refining the stellar properties of the \gw\ system will enable important tests of the pre-main sequence evolutionary models at the high mass initial mass function.
...
We report the first resolved sub-mm observations of the \gw\ system, and report the presence of a large and massive protoplanetary disk. We forward-model the protoplanetary disk to infer the total system mass of the enclosed stars, since these dominate the gravitational field \citep[][e.g.]{czekala15a,czekala16}. We combine these mass constraints with measurements of the photospheric properties of A to evaluate the agreement of A with leading pre-main sequence models. We discuss the disk geometry in the presence of observations about the eclipsing behavior and possible analogy to K2 dipper stars.
