...
Although we do not directly measure visible light from any stars in the GW~Ori system other than the A component, the existence of a B component was long known from radial velocity monitoring \citep{mathieu91} and the C component had long been suspected. In 2011, \citet{berger11} used the IOTA/IONIC3 interferometer to reconstruct an image that reveals all three components, as well as measure flux ratios in H-band. It is interesting to discuss the inferred dynamical masses of these components with the measured H-band fluxes of Berger and the non-detections of spectral lines in the visible.

In Figure~\ref{fig:evolution}, we plot the ratio of effective temperatures and the computed flux ratios in Bessell I band and 2MASS H band. Bessell I roughly corresponds to the reddest echelle orders of the TRES spectrograph that we used to search for companion spectral lines.

% Berger's ratios.
The mean H-band flux ratio (computed as the weighted mean of all measurements) is $f_B/f_A = 0.57 \pm 0.05$ and $f_C/f_A = 0.23 \pm 0.01$. This means that the excess H-band flux is likely due to the presence of a circumstellar disk or accretion signatures above the photoshpheric emission of these stars, assuming that all stars are the same age.

To our knowledge there has been no radial velocity monitoring of GW~Ori in the infrared.


\begin{figure}[htb]
\begin{center}
  \includegraphics{evolution.pdf}
  \figcaption{MIST PMS tracks showing photospheric properties as function of time. Under the assumption that all three stars are the same age, flux contrasts in Bessell I band and 2MASS H band are computed.
  \label{fig:evolution}}
  \end{center}
\end{figure}
