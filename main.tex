...
\title{Evidence for Disk--Stellar Mutual Inclinations in the Young and Massive GW Ori Triple System}
...
\affiliation{Hubble, Carnegie-Princeton Fellow}
...
%\author{ASAS-SN Co-Authors}
%Benjamin J. Shappee$^{4,5}$,
%Krzysztof Z. Stanek$^{2,15}$, 
%Thomas W.-S. Holoien$^{2,15,19}$, 
%Jose L. Prieto$^{20,21}$, 
%Christopher S. Kochanek$^{2,15}$, 
...
\tablehead{\colhead{Parameter} & \colhead{Value} & \colhead{Unit}}
\startdata
...
\enddata
\tablecomments{Here the inclinations are limited to be between $0^\circ$ and $90^\circ$, although they apply equally to inclinations between $90^\circ$ and $180\circ$, since the RV data can not constrain the absolute inclination of the orbit.}
\end{deluxetable}
...
% This needs to be changed from "evaluating the models" to instead "what are the stellar properties?" The fact that this is a triple and we don't have good photospheric constraints nor truly solid orbital constraints, means that we are not able to "evaluate" the PMS models the same way we did for AK Sco and DQ Tau. Instead, I think we need to take the previous mass and inclination meauserements, and then discuss where to place these stars on the HR diagram.

% First, discuss Keivan's compilation of the SED, and how he did the fitting.
% Discuss how the spectroscopic constraints give us a rough estimate of V-band flux ratio

% Do a fit for using age, Teff, M_A, M_B, M_C and L_star using the MIST models. As constraints, we have total luminosity, H-band flux ratio, and broad posteriors on T_eff1 and T_eff2. Look in Master-crib.py first to see what's been done. Then, compare the posteriors on the masses to the posteriors on the masses from the RV + Disk fits.

% Figure: Use posteriors on Teff, L to plot A, B, and C on an HR diagram.


% Redo figure 14 (the Teffs and f_ratios as a function of age) using the mean best predictions for the stellar properties. 
