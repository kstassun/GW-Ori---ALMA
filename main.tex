...
\todo{Expand text}
Alternative radial velocity analysis using the \citet{fang14} points.
Scaled by an inferred offset. Notable changes from the CfA only no-jitter orbit are
$K_\mathrm{A} = 3.28 \pm 0.08\,\kms$, $e_\mathrm{inner} = 0.05 \pm 0.03$, $K_\mathrm{AB} = 2.27 \pm 0.13\,\kms$, $e_\mathrm{outer} = 0.29 \pm 0.05\,\kms$, for a jitter value of $\sigma_\mathrm{jitter} = 0.55 \pm 0.05\,\kms$.

We also perform an additional orbital fit using the CfA velocities, \citet{fang14} radial velocities, and the three astrometric measurements in \citet{berger11}.
The addition of the astrometric points allow us to constrain $a_\mathrm{inner} = 1.24 \pm 0.02\,\mathrm{AU}$, $i_\mathrm{inner} = 155 \pm 8^\circ$, $\Omega_\mathrm{inner} = 265 \pm 12^\circ$, $a_\mathrm{outer} = 9.1 \pm 0.1\,\mathrm{AU}$, $i_\mathrm{outer} = 153 \pm 6^\circ$, and
$\Omega_\mathrm{outer} = 266 \pm 7^circ$ and constrains masses to be $M_A = 3.72 \pm 0.3 M_\odot$, $M_B = 0.66 \pm 0.20 M_\odot$, and $M_C = 1.13 \pm 0.23 M_\odot$. This orbit yields acceptable residuals to all points within the measurement uncertainties, although we note that the model predicts separations $\rho_\mathrm{inner}$ that are all smaller than the measurements. We are skeptical of the results of this fit, compared to the next section, the disk inclination is off.
...
% Which is more consistent with the predictions from the evolutionary models, but predicts a 45 degree misalignment between the inner orbit and the disk, and a similar misalignment between the outer orbit and the disk.
